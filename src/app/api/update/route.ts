...
export async function POST(req: Request) {
  ...
  if (
    ...
    return NextResponse.json({ response: 'Invalid data, must be a number' }, { status: 200 })
  } else {
  ...
  const responseSha = await fetch('https://api.github.com/repos/rice-apps/united-way/contents/public/data.json', {
    method: 'GET',
    headers: {
  ...
  const response = await fetch('https://api.github.com/repos/rice-apps/united-way/contents/public/data.json', {
    method: 'PUT',
    headers: {
      Accept: 'application/vnd.github+json',
      Authorization: `Bearer ${token}`,
